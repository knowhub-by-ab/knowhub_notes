...
graph TD
    ...
    B --> C(Axon)
    C --> D[Axon Terminals]
    D -- Neurotransmitters --> E(Synapse)
    E --> F[Dendrite of next Neuron]
...
```mermaid
graph LR
    A[Presynaptic Neuron Axon Terminal] -- Release Neurotransmitters --> B(Synaptic Cleft)
    B -- Neurotransmitters Bind To --> C[Postsynaptic Neuron Dendrite Receptors]
    C -- Chemical Signal --> D(Electrical Signal)
