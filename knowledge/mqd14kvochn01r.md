...
```mermaid
graph TD
    A[Raw Information Acquisition] --> B{Pattern Recognition};
    B --> C{Relationship Building};
    C --> D{Integration};
    D --> E[Consolidation & Refinement];
    E -- New Experience / Information --> A;

    A -- Sensory Input, Learning --> B;
    B -- Identifying Similarities, Differences --> C;
    C -- Connecting Concepts, Cause/Effect --> D;
    D -- Fitting into Existing Structures, Modifying --> E;
    E -- Practice, Reflection, Testing --> A;
```
...
```mermaid
graph TD
    A[Animal Schema] --> B[Bird Schema];
    A --> C[Mammal Schema];
    B --> D[Sparrow Schema];
    B --> E[Eagle Schema];
    B --> F[Penguin Schema];
    D -- Has-a --> G[Feather Schema];
    E -- Behavior --> H[Hunting Schema];
    F -- Environment --> I[Cold Climate Schema];
    B -- Action --> J[Flying Schema];
    J -- Counter-example --> F;
    G -- Part-of --> K[Wing Schema];
    H -- Skill --> L[Predation Schema];

    subgraph Related Concepts
        M[Ecology Schema]
        N[Evolution Schema]
        O[Biomechanics Schema]
    end
